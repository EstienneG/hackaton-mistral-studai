...
for key, value in answers.items():
    options.append({"id": key + "_" + value["correct"], "text": value["correct"]})
    options.append({"id": key + "_" + value["false"], "text": value["false"]})
...
drag_and_drop_html = f"""
    <div>
        ...
        <script>
            ...
            function updateAnswers(blankId, answerId) {{
                var answers = JSON.parse(localStorage.getItem("answers")) || {{}};
                answers[blankId] = answerId;
                localStorage.setItem("answers", JSON.stringify(answers));
                document.getElementById("answerData").value = JSON.stringify(answers);
            }}
...
drag_and_drop_html += f"""
        ...
        <form action="/submit" method="post">
            <input type="hidden" id="answerData" name="answerData" value="{{}}">
