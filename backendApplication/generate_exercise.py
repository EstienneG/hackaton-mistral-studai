...
def init_fill_gap_writer(llm:ChatMistralAI, revtrieved_chunks:str):
...
def init_qcm_writer(llm: ChatMistralAI, retrieved_chunks: str):
    # Define the template for the MCQ Writer
    system_message_mcq_choice = f"""
    You are the exercise creator. Your task is to generate a multiple-choice exercise based on a given chapter of a book. Each question should have 4 options with only one correct answer. The exercise should be relevant to the chapter's topic and designed to reinforce the reader's understanding of the material.
    ...
    You Must always create your 3 questions at all costs. if you put more than 3 questions or less than 3 questions, your generation is considered as incorrect.
    ...
    exercise_template = ChatPromptTemplate.from_messages([("system", system_message_mcq_choice), ("human", human_message)])

    # Create a memory for the MCQ Writer
    exercise_memory = ConversationBufferMemory()

    # Create the MCQ Writer chain
    exercise_writer_chain = LLMChain(
        llm=llm,
        prompt=exercise_template,
        memory=exercise_memory
...
def generate_fill_gap(revtrieved_chunks:str, difficulty:str):
    ...
    writter = init_fill_gap_writer(llm, revtrieved_chunks)
    critic = init_critic(llm, revtrieved_chunks)

    return invoke_chain_of_agents(difficulty, writter, critic)

def generate_qcm(revtrieved_chunks:str, difficulty:str):
    revtrieved_chunks = revtrieved_chunks.replace('{', '{{').replace('}', '}}')

    llm = ChatMistralAI(api_key = "ImsUHfFLA6OjlX6mARbnM1YcDOy7ujsq", model = "open-mixtral-8x22b", temperature=0)
    writter = init_qcm_writer(llm, revtrieved_chunks)
    critic = init_critic(llm, revtrieved_chunks)
